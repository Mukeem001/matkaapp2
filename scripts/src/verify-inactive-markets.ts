...
async function verifyInactiveMarkets() {
  console.log("🔍 Verifying INACTIVE/CLOSED Markets Results for Today");
  ...
  const inactiveMarkets = await db
    .select()
    .from(marketsTable)
    .where(eq(marketsTable.isActive, false));

  console.log(`Found ${inactiveMarkets.length} inactive/closed markets\n`);
  ...
  // Summary
  console.log("\n\n" + "=".repeat(70));
  console.log("📋 INACTIVE MARKETS - VERIFICATION SUMMARY");
  console.log("=".repeat(70));
  ...
  if (mismatchCount > 0) {
    console.log("\n\n" + "=".repeat(70));
    console.log("🔴 MISMATCHED INACTIVE MARKETS");
    console.log("=".repeat(70));
  ...
  if (noResultMarkets.length > 0) {
    console.log("\n\n" + "=".repeat(70));
    console.log("⚠️  INACTIVE MARKETS WITH NO RESULTS SAVED");
    console.log("=".repeat(70));
    ...
    noResultMarkets.forEach(report => {
      ...
      console.log(`  Note: Market is inactive, so no auto-update occurs`);
    });
  }
...
verifyInactiveMarkets().catch(console.error);
